...
open class StatesQ32int : SizeBase() {
    ...
    abstract class RefState : SizeState() {
        @JvmField var arr: Array<Q32int?>? = null
        fun fill() {
            for (i in arr!!.indices) {
                arr!![i] = Q32int(i)
            }
    ...
    open class Ref_as_Obj : ObjState() {
        @Setup
        fun setup() {
            arr = arrayOfNulls<Q32int>(size) as Array<Any?>
            fill()
        }
    }
    ...
    open class Ref_as_Int : IntState() {
        @Setup
        fun setup() {
            arr = arrayOfNulls<Q32int>(size) as Array<Int32?>
            fill()
        }
    }
    ...
    open class Val_as_By : ByState() {
        @Setup
        fun setup() {
            arr = arrayOfNulls<Q32int>(size) as Array<ByInt?>
            fill()
        }
    }

...
open class StatesQ64int : SizeBase() {
    ...
    abstract class RefState : SizeState() {
        @JvmField var arr: Array<Q64int?>? = null
        fun fill() {
            for (i in arr!!.indices) {
                arr!![i] = Q64int(i.toLong())
            }
    ...
    open class Ref_as_Obj : ObjState() {
        @Setup
        fun setup() {
            arr = arrayOfNulls<Q64int>(size) as Array<Any?>
            fill()
        }
    }
    ...
    open class Ref_as_Int : IntState() {
        @Setup
        fun setup() {
            arr = arrayOfNulls<Q64int>(size) as Array<Int64?>
            fill()
        }
    }
    ...
    abstract class ByState : SizeState() {
        ...
        fun fill() {
            for (i in arr!!.indices) {
                arr!![i] = Q64int(i.toLong()) as ByInt
            }
        }
    }
    ...
    open class Val_as_By : ByState() {
        @Setup
        fun setup() {
            arr = arrayOfNulls<Q64int>(size) as Array<ByInt?>
            fill()
        }
    }

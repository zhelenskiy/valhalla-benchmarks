...
open class StatesROpt : SizeBase() {
    ...
    abstract class RefState : SizeState() {
        @JvmField var arr: Array<ROpt<Int32>?>? = null
        fun fill() {
            for (i in arr!!.indices) {
                arr!![i] = ROpt.of(R32int(i))
            }
    ...
    open class Ref_as_Obj : ObjState() {
        @Setup
        fun setup() {
            arr = arrayOfNulls<ROpt<*>>(size) as Array<Any?>
            fill()
        }
    }
    ...
    open class Ref_as_Int : IntState() {
        @Setup
        fun setup() {
            arr = arrayOfNulls<ROpt<*>>(size) as Array<Opt<Int32>?>
            fill()
        }
    }
    ...
    open class Ref_as_Ref : RefState() {
        @Setup
        fun setup() {
            arr = arrayOfNulls<ROpt<*>>(size) as Array<ROpt<Int32>?>
            fill()
        }
    }
}

...
open class StatesR128int : SizeBase() {
    ...
    abstract class RefState : SizeState() {
        @JvmField var arr: Array<R128int?>? = null
        fun fill() {
            for (i in arr!!.indices) {
                arr!![i] = R128int(i.toLong())
            }
    ...
    class Ref_as_Obj : ObjState() {
        @Setup
        fun setup() {
            arr = arrayOfNulls<R128int>(size) as Array<Any?>
            fill()
        }
    }
    ...
    class Ref_as_Int : IntState() {
        @Setup
        fun setup() {
            arr = arrayOfNulls<R128int>(size) as Array<Int128?>
            fill()
        }
    }

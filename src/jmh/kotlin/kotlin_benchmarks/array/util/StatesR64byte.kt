...
open class StatesR64byte : SizeBase() {
    ...
    abstract class RefState : SizeState() {
        @JvmField var arr: Array<R64byte?>? = null
        fun fill() {
            for (i in arr!!.indices) {
                arr!![i] = R64byte(i.toLong())
            }
    ...
    class Ref_as_Obj : ObjState() {
        @Setup
        fun setup() {
            arr = arrayOfNulls<R64byte>(size) as Array<Any?>
            fill()
        }
    }
    ...
    class Ref_as_Int : IntState() {
        @Setup
        fun setup() {
            arr = arrayOfNulls<R64byte>(size) as Array<Int64?>
            fill()
        }
    }
